...
PDF에 나가는 항목:
- 회사 정보 (이름, 사업자등록번호, 연락처, 주소)
- 견적일 / 유효기간
- 고객 정보 / 현장 주소
- 공사명 / 공사 유형 / 시공·건물 면적 / 공사 일정 ("YYYY년 MM월 중")
- 공사 범위 (체크된 항목을 사람이 읽을 수 있는 문구로 변환)
- 사용 자재 pill (제품명 | 두께 | 텍스처 | 색상)
- **견적 내역 테이블** — 두 모드:
  - **간단 내역**: 4~5개 그룹 (자재 및 마감 / 시공비 / 장비 및 운송 / 철거 및 폐기 / 기타)
  - **상세 내역**: 자재 항목 하나씩 + 인건비/식비/숙박비는 "시공비"로 묶음 (내부 비용 노출 금지)
- 합계 (최종가 한 줄, 옆에 "(부가세 포함)" 또는 "(부가세 별도)" — 공급가/VAT 별도 표시 안 함)
- 결제 조건 / 안내 문구
- 맨 아래 "위와 같이 견적합니다." + 직인 (이미지 또는 placeholder)
