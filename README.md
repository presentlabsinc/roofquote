...
새 견적 만들기 화면 (`/sites/[id]/estimates/new`) 9단계:
...
5. **색상 / 텍스처** — 9가지 프리셋 (다크브라운 기본) + 기타 직접 입력
6. **공사 범위** — 공사 유형에 따라 옵션 다름:
   - `roof`: 덧씌우기, 철거, 용마루, 처마, 물받이(+길이), 폐기물
   - `rooftopRoof`: 골조보강, 용마루, 처마, 물받이(+길이), 창고(+면적), 계단실(+면적), 폐기물
   - `steelWaterproof`: 난간및두겁, 기존방수재철거, 배수구, 창고(+면적), 계단실(+면적), 폐기물
7. **장비대** — 스카이차/사다리차/비계 각각 사용일수 + 기타 장비 메모
8. **작업 정보** — 작업 일수, 작업 인원
9. **기타 비용** — 자유 추가 라인아이템 (이름 + 금액). 크레인, 추가 자재, 잡비 등

가격 처리:
- **두께**: 0.45t 기준에 단순 배수 적용 (0.4t ×0.92, 0.5t ×1.08, 0.6t ×1.22 — [lib/calculations.ts](lib/calculations.ts) 의 `THICKNESS_MULT`)
- **난간 및 두겁**: 체크 시 자재 면적이 자동으로 `parapetMultiplier` 배 (기본 1.4×) 적용됨. 단가 설정에서 조정 가능.
- **창고/계단실**: 추가 면적 × 자재 단가로 별도 라인아이템 생성.
